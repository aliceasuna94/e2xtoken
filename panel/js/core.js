    ...
    loginPromise.then((result) => {
        return new Promise(async (resolve, reject) => {
            ...
            web3.eth.getAccounts().then(function (result) {
                ...
                setUpContracts()
                if ($('.ref-link')[0]) $('.ref-link')[0].value = "https://e2xtoken.io/?ref=" + user.address
            })
...
function getUserBalance() {
    mainContract.methods.balanceOf(user.address).call({
        ...
        user.balance = res
        if ($('.your-token-balance-hd')[0]) $('.your-token-balance-hd')[0].innerHTML = "Your E2X balance: " + (user.balance / 100000000).toLocaleString()
    })
}
